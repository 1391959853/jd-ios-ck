...
function getPinMap() {
    let raw = $persistentStore.read("JD_PinMap");
    return raw ? JSON.parse(raw) : {};
}
function savePinMap(map) {
    $persistentStore.write(JSON.stringify(map), "JD_PinMap");
}
...
function checkIfProcessed(pt_pin, pt_key, wskey) {
    try {
        let raw = $persistentStore.read("JD_Processed_Records");
        if (!raw) return false;
...
function recordProcessed(pt_pin, pt_key, wskey) {
    try {
        ...
        $persistentStore.write(JSON.stringify(data), "JD_Processed_Records");
    } catch (e) {}
}
...
function submitToAPI(pt_pin, pt_key, wskey, cookie, pin_hash) {
    ...
    $httpClient.post(API_URL, headers, body, function(error, response, data) {
        if (error) {
        ...
        console.log(`API返回状态码: ${statusCode}`);
        let responseBody = data || "";
        console.log(`API返回完整内容:\n${responseBody}`);

        // 检查是否返回“校验失败，京东账号: xxx”
        if (responseBody.startsWith("校验失败，京东账号: ")) {
            let match = responseBody.match(/校验失败，京东账号: (.+)/);
        ...
        // 原有的通知逻辑
        if (responseBody.includes("ok")) {
            console.log(`✅ Cookie提交成功`);
            sendLocalNotification("API提交成功", `账号: ${pt_pin}`, responseBody);
        } else {
            console.log(`❌ API返回失败: ${responseBody}`);
            sendLocalNotification("API提交失败", `账号: ${pt_pin}`, responseBody);
        }
        pendingAsyncTasks--;
        if (pendingAsyncTasks === 0) $done({});
    });
